...
//---------------------------------------------------------------------------
// ######## Hidden channel detect for OS
//---------------------------------------------------------------------------
void CheckOther::checkHiddenChannel()
{
    int flag = 0;
    const Token* atoken = NULL;
    const SymbolDatabase *symbolDatabase = mTokenizer->getSymbolDatabase();
    for (const Scope * scope : symbolDatabase->functionScopes) {
        if(scope->className == "alloc_inode"|| scope->className == "vfs_create"||scope->className == "may_create"||scope->className == "alloc_empty_file"){
            atoken = scope->bodyStart;
            for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
                if(tok->str() == "return"){
                    const Token* tokentmp1 = tok->tokAt(-6);
                    const Token* tokentmp2 = tok->tokAt(-5);
                    // tokentmp = tok->tokAt(-6);
                    // const char * c = tok->str().c_str();
                    // printf(c);
                    // printf(c);
                    if(tokentmp1->str() == "DELAY"|| tokentmp2->str() == "DELAY"){
                        // printf("find delay");
                        flag = 1;
                        break;
                    }
            ...
            if(!flag){
                // printf("atoken is not null");
                HiddenChannelError(atoken);
            }
        }
    }
}
    ...
    //             flag = 1;
    //     }
void CheckOther::HiddenChannelError(const Token *tok)
{
    reportError(tok,
                Severity::error,
                "HiddenChannelThreat",
                "function 'delay()' is required to avoid Hidden Channel Threat",CWE369,false);
}
...
// void CheckOther::CheckMemoryInit(){
//     const SymbolDatabase *symbolDatabase = mTokenizer->getSymbolDatabase();
//     for (const Scope * scope : symbolDatabase->functionScopes) {
//         for(const  Token *tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()){
//             if(Token::Match(tok, "struct %name% %var% ; ")){
//                 int flag = 0;
//                 const Token *tok_now = tok->tokAt(2);
//                 const unsigned int memsetID = tok_now->varId();
//                 // const Token *toktmp = tok->tokAt(2)->;
//                 // printf(tok->str().c_str());
//                 for(const Token *toktmp = tok; toktmp != scope->bodyEnd; toktmp = toktmp->next()){
//                     if(Token::Match(toktmp, "memset ( %op%| %varid% , ", memsetID)){
//                         flag = 1;
//                         break;
//                     }
//                 }
//                 if(!flag){
//                     CheckMemoryInitError(tok_now);
//                 }
//             }
//         }
//     }
...
// void CheckOther::CheckMemoryInitError(const Token *tok){
//     reportError(tok,
//         Severity::error,
//         "MemoryNotInit",
//         "the struct object ' " + tok->str() + " ' hasn't initialized which will could cause MemoryLeak problems...", CWE(0U),false);
// }

void CheckOther::checkFive(){
    const SymbolDatabase *symbolDatabase = mTokenizer->getSymbolDatabase();
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for(const  Token *tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()){
            // if(tok->varId() > 0 && Token::Match(tok,"%name% [ %name% ] =|;")){
            //     if(Token::Match(tok->previous(),"%name%"))
            //         continue;
            //     int outofboundflag = 0;
            //     const Token *atok = tok->tokAt(2);
            //     const unsigned int indexId = atok->varId();
            //     for(const Token *newbegin = scope->bodyStart->next(); newbegin != tok; newbegin = newbegin->next()){
            //         if(indexId > 0 && (Token::Match(newbegin,";|<|<= %varid% <|<=", indexId)||Token::Match(newbegin,"%varid% >|>=", indexId))){
            //             outofboundflag = 1;
            //             break;
            //         }
            //     }
...
// void CheckOther::OutofBoundError(const Token *tok){
//     reportError(tok,
//         Severity::error,
//         "ArrayIndexOutofBound",
//         "The Array ' " + tok->str() + " ' index may be invalid and could cause OutofBound Error",CWE(0U),false);    
// }
void CheckOther::DangerFunctionError(const Token *tok){
    reportError(tok,
        Severity::error,
        "DangerousFunction",
        "' " + tok->str() + " ': Use of Potentially Dangerous Function.",CWE(0U),false);
}
...
void CheckOther::FileOpError(const Token *tok){
    reportError(tok,
        Severity::error,
        "FileOprationError",
        tok->str() + " : File Open without close.",CWE(0U),false);
}
